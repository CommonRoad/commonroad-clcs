...
Eigen::Vector2d Segment::tangentSegmentStart() const { return this->t_1_; }

Eigen::Vector2d Segment::tangentSegmentEnd() const { return this->t_2_; }
...
double Segment::computeLambda(const Eigen::Vector2d& p_local) const {
  // see Eq. (7) in Hery et al. (2017): Map-based curvilinear coordinates for autonomous vehicles
  double lambda = -1;
...
Eigen::Vector2d Segment::rotateToLocalFrame(const Eigen::Vector2d& p) const {
  Eigen::Matrix2d R;
  R << this->tangent_[0], this->tangent_[1],
       this->normal_[0], this->normal_[1];
  return R * p;
}

Eigen::Vector2d Segment::rotateToGlobalFrame(const Eigen::Vector2d& p_local) const {
  Eigen::Matrix2d R;
  R << this->tangent_[0], this->normal_[0],
       this->tangent_[1], this->normal_[1];
  return R * p_local;
}
